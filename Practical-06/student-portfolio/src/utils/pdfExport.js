...
export function exportTasksToPDF(tasks, filterTab = "all") {
  const doc = new jsPDF();
  ...
  doc.text("Task Management System - Practical 6", 14, 20);

  doc.setFontSize(10);
  doc.setFont("helvetica", "normal");
  doc.setTextColor(100, 116, 139);
  doc.text(`Full Stack Integration: React + Express + MongoDB Atlas`, 14, 26);
  doc.text(`Generated on: ${new Date().toLocaleString()} | Active Filter: ${filterTab.toUpperCase()}`, 14, 32);
  ...
  for (let i = 1; i <= pageCount; i++) {
    ...
    doc.text(
      `Page ${i} of ${pageCount} — Advanced Web Development Frameworks Practical 6`,
      14,
